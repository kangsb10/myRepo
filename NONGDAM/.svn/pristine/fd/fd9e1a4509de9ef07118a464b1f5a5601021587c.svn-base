...
// 	DB에서 textarea를 <br> 없이 가져오는거 
	 ...
	 $('form[name=updateForm]').submit(function(){
	      ...
	      if($('#cstmr_cnter_cn').val() == null || $('#cstmr_cnter_cn').val().trim() == ""){
		         alert("내용을 입력하세요");
		         return false;
		      }
	      
	      $(this).attr('action', '${pageContext.request.contextPath}/user/customercenter/updateStatement.do'); 
...
// 	      $(this).append('<textarea type="hidden" name="ctvt_mth_cn"></textarea>' );
	...
	$('#cancelbt').click(function() {
		...
		$(location).attr('href', '/user/customercenter/statementView.do?mber_id=' + mber_id + '&cstmr_cnter_no=' + cstmr_cnter_no + '&search_keyword=' + search_keyword + '&search_keycode=' +search_keycode+ '&currentPage=' + currentPage);
	});
