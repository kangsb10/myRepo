...
<button type="button" id="insertBt" style=" color: #fff; background: #E16422; border-radius: 20px; padding: 5px 12px 5px 12px; border: 0; position: relative; left: 170px; top: 0px;" >+</button>
	<div class="" >
    	<div class="" style="font-size: 30px; color:#0B3B24;" >같이키워요</div>
		<table class="" style="position: relative; top: 20px; ">
			...
			<c:choose>
				<c:when test="${empty cultivationmethodList }">
					<tr>
						<td colspan="5" align="center"><font color="red">등록된 게시글이 없습니다.</font></td>
					</tr>
				</c:when>
				<c:otherwise>
					<c:forEach items="${cultivationmethodList }" var="cultivationmethodInfo">
						<tr>
							<td><input type="hidden" value="${cultivationmethodInfo.ctvt_mth_no}">${cultivationmethodInfo.rnum}</td>
							<td>${cultivationmethodInfo.ctvt_mth_sj}</td>
							<td>${cultivationmethodInfo.mber_ncnm}</td>
							<td style="font-size: 5px;">${cultivationmethodInfo.ctvt_mth_input_de}</td>
							<td>${cultivationmethodInfo.ctvt_mth_hit}</td>
						</tr>
					</c:forEach>
				</c:otherwise>
			</c:choose>
...
<form action="/user/cultivationmethod/cultivationMethodList.do" method="post" class="form-inline">
...
$(function(){
	...
	$("#cultlvationMethodTBL tr").click(function(){
		var ctvt_mth_no = $(this).find("td:eq(0) input").val();
		var search_keyword = encodeURIComponent('${search_keyword}');
		var search_keycode = '${search_keycode}';
		var currentPage = '${currentPage}';
		$(location).attr("href", "${pageContext.request.contextPath}/user/cultivationmethod/cultivationMethodView.do?ctvt_mth_no=" + ctvt_mth_no + '&search_keyword=' + search_keyword + '&search_keycode=' +search_keycode+ '&currentPage=' + currentPage);		
		
	});
	
	$('#insertBt').click(function() {
		$(location).attr('href','${pageContext.request.contextPath}/user/cultivationmethod/cultivationMethodForm.do');
	});
