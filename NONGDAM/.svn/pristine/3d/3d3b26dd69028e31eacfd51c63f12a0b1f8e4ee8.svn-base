...
<form name="updateForm" method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath}/user/sfms/dataExtrac.do">
	<div>
		<ul >
...
<!--                         </div> -->
						...
						<c:if test="${farmInfo.farm_crops eq '토마토'}">
							<img src="/img/sfms/sfms_view.jpg" alt="" style=" width:380px; height:180px; border-radius: 25px; ">
						</c:if>
<%-- 						<c:if test="${empty sensorInfo.items }"> --%>
...
<%-- 						</c:if> --%>
	...
	<div style="float: left; padding-top: -15px; padding-bottom: 10px;">
         ...
         <div class="" style=" float: left; margin-left: 70px; margin-top: 5px;"  >
			 <input type="range" min="1" max="50" value="${farmInfo.farm_tmprt }" class="slider" id="myRange_thermo">
				
		 </div>
		 <div style=" float: left; margin-left: 10px; "><p style="margin-top: -1px;">온도 :<span class="sp" id="demo_thermo" style="margin-left: 5px; "></span><span class="sp" style="font-size: 20px;"> &deg;</span></p>
		 </div>
	</div>
	<!-- 습도설정 -->
	<div style="float: left; padding-top: -5px; padding-bottom: 10px;">
         ...
         <div class="" style=" float: left; margin-left: 70px; margin-top: 5px;"  >
			 <input type="range" min="1" max="100" value="${farmInfo.farm_hd }" class="slider" id="myRange_hum">
				
		 </div>
		 <div style=" float: left; margin-left: 10px; "><p style="margin-top: -1px;">습도 : <span class="sp" id="demo_hum" style="margin-left: 5px; "></span><span class="sp" style="font-size: 17px;">&#37;</span></p>
		 </div>
	</div>
	...
	<div style="float: left; padding-top: -5px; padding-bottom: 10px;">
         ...
         <div class="" style=" float: left; margin-left: 70px; margin-top: 5px;"  >
			 <input type="range" min="1" max="100" value="${farmInfo.farm_co2 }" class="slider" id="myRange_co2">
				
		 </div>
		 <div style=" float: left; margin-left: 10px; "><p style="margin-top: -1px;">CO2 : <span class="sp"  id="demo_co2" style="margin-left: 5px; "></span><span class="sp" style="font-size: 15px;">&#112;&#112;&#109;</span></p>
		 </div>
	</div>
	...
	<div style="float: left; padding-top: -5px; padding-bottom: 8px;">
         ...
         <div class="" style=" float: left; margin-left: 70px; margin-top: 5px;"  >
			 <input type="range" min="1" max="50" value="${farmInfo.farm_pblmrl }" class="slider" id="myRange_wnd">
				
		 </div>
		 <div style=" float: left; margin-left: 10px; "><p style="margin-top: -1px;">풍속 : <span class="sp" id="demo_wnd" style="margin-left: 5px; "></span><span class="sp" style="font-size: 15px;">&#109;&#47;&#115;</span></p>
		 </div>
	</div>
					...
					<input type="hidden" value="${farmInfo.mber_id}" name="mber_id"/>
					
	<div style="float: left; position: relative; left: 8px; top: 20px;">
	...
	<div style="float: left; position: relative; left: 8px; top: 20px;">
		<button type="button" id="deleteBt" style=" color: #fff; background: #E16422; border-radius: 20px; padding: 5px 12px 5px 12px; border: 0;" >농장삭제</button>
	</div>
	<div style="float: left; position: relative; left: 8px; top: 20px;">
 ...
 $(function(){
	...
	$('#sensor_pblmrl').text(${sensorInfo.sensor_pblmrl  });
	 ...
	 $('#deleteBt').click(function(){//삭제
	          $(location).attr('href', '${pageContext.request.contextPath}/user/sfms/deleteFarm.do?farm_id=${farmInfo.farm_id}&mber_id=${farmInfo.mber_id}');
	    });
	 
	/* 농장 목록가기 */
	$('#listBt').click(function() {
			$(location).attr('href', '/user/sfms/farmList.do?mber_id=${farmInfo.mber_id}');
	});
	// 데이터테스트
...
// 						tmprtloop(i,sensor_tmprt);
				   ...
				   for(var i = $('#sensor_tmprt').text(); i>=farm_tmprt; i--){
					   ...
					   if(i == farm_tmprt){
						   time = "마지막회차"
						   tmprtloops(i,sensor_tmprt,time);
					   }else{
						   tmprtloops(i,sensor_tmprt,time);
	...
	//온도 올랐을때
	function tmprtloop(i,temp,time){
		setTimeout(function(){
			var value = i;
			var text = "";
		if(time =="마지막회차"&& i >"${item.stndTemp }"){
			alert("온도가 너무 높습니다");
			text = "온도가 너무 높습니다 온도를 낮춰주세요";
					$.ajax({
						type : 'POST',
						url : '${pageContext.request.contextPath}/user/farmingdiary/updateFarmDiary_Sfms_Emrgncy_cn.do',
						dataType : 'json',
						data : { farm_diary_sfms_emrgncy_cn : text , mber_id : "${farmInfo.mber_id}" },
						error : function(result){
							alert(result.status + ' | ' + result.responseText);
						},
						success : function(result){
		...
		}else if(time =="마지막회차"&&i<15){
			alert("온도가 너무 낮습니다");
			text="온도가 너무 낮습니다 온도를 높여주세요";
				$.ajax({
					type : 'POST',
					url : '${pageContext.request.contextPath}/user/farmingdiary/updateFarmDiary_Sfms_Emrgncy_cn.do',
					dataType : 'json',
	...
	// 온도 떨어졌을때
	function tmprtloops(i,temp,time){
		setTimeout(function(){
			var value = i;
			var text = "";
		if(time =="마지막회차"&&i<15){
		...
		}else if(time =="마지막회차"&& i >"${item.stndTemp }"){
			alert("온도가 너무 높습니다");
			text = "온도가 너무 높습니다 온도를 낮춰주세요";
			$.ajax({
				type : 'POST',
