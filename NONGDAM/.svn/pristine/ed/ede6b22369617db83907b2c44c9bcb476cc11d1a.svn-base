...
<form name="insertForm"  class="form-horizontal"  method="post" action="/user/cm/updateCafeNCNM.do" >
<div id="" style="position: relative; top: 70px;">
   <div class="">
      <ul >
         ...
         <li style="position: relative; left: 10px; top: -5px; font-size: 20px; color: #0B3B24; float: left;"> 수정하기</li>
      </ul>
                    ...
                    <div class="p-ins">
                        <div class="row">
                        	<ul style="margin-top: 50px;">
								...
								<li style="float:left; padding-left: 18px;"> <input type="text" value="${cafeName.cafe_mber_ncnm }" class="form-control" name="cafe_mber_ncnm" id="cafe_mber_ncnm"  placeholder="카페에서 사용할 별명" style="position: relative; top:-5px; width: 150px;  height: 30px;">
...
//취소버튼누르면 카페메인가게 잘해봐
   ...
   $('form').submit(function(){
	//	$(this).append('<input type="hidden" name="mber_id" value="${memberInfo.getMem_id()}"/>');
		$(this).append('<input type="hidden" name="mber_id" value="${communityInfo.mber_id}"/>');
