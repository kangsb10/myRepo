			...
			<c:choose>
				...
				<c:otherwise>
					<c:forEach items="${customerCenterList }" var="customerCenterInfo">
						<tr>
							<td><input type="hidden" value="${customerCenterInfo.cstmr_cnter_no}">${customerCenterInfo.rnum}</td>
...
<form action="/admin/customercenter/customerCenterList.do" method="post" class="form-inline">
...
$(function(){
	...
	$("#customerCenterTBL tr").click(function(){
		...
		$(location).attr("href", "${pageContext.request.contextPath}/admin/customercenter/customerCenterView.do?cstmr_cnter_no=" + cstmr_cnter_no);		
