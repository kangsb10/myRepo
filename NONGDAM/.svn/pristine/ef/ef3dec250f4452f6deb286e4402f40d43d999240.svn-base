                ...
                <div class="col-lg-8">
                    <div class="p-ins">
                        ...
                        <ul style="margin-top: 50px;">
							...
							<li style="float:left; padding-left: 18px;"> <input type="text" class="form-control" id="bbs_creat_sj" name="bbs_creat_sj" placeholder="제목 입력" style="position: relative; top:-5px; width: 100px;  height: 30px;"></li>
						</ul>
	                     </div>
                    </div>
                </div>
...
// 	});
	...
	$('#listbt').click(function() {
	 ...
	 $('form[name=insertForm]').submit(function(){
	      ...
	      $(this).attr('action', '/user/cm/cafeBoardIns.do?cmmnty_cafe_adres=${communityInfo.cmmnty_cafe_adres}'); 
	      $(this).append('<input type="hidden" name="cmmnty_no" value="${communityInfo.cmmnty_no}"></input>');
	      $(this).append('<input type="hidden" name="cafe_mber_id" value="${login_mberInfo.mber_id}"></input>');
	   });
});
</script>
