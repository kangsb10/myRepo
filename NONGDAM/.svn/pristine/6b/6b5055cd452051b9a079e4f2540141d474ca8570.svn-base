...
<!-- 	<div customattr="spotFore" style="display:none;"></div>  -->
     <div customattr="areaWeather" style="display:none;"> 
      <h2></h2> 
      <p></p> 
      <div class="weather" style=" z-index: 400"> 
       <!-- ajax --> 
       <span></span>
...
<!--                   <a href="single-post.html" style="position: relative; top: -40px;"><img src="img/main/weath.png" class="img-responsive" alt="Blog"></a> -->
                    <div style="margin-top: -40px; margin-bottom: 10px;  width: 400px; height: 50px;">
	                    <ul style="margin-left: 20px;">
	                    <li style="float: left;"><div style="  position:relative; left:; background-color:; margin-top: -8px;"><img src="" id="todaysWeather" style="width: 70px;"></div></li>
	                    <li style="float: left; margin-left: 10px; margin-top: 15px;"><div class="post-format" style=" position:relative; margin-left:; margin-top: 0px;  width: 200px;">
	                          <span style="color: #FEEACC; float:left;">기온 <a href="#" style="color: #FFFFFF;" id="todaysTemp">28도</a>℃</span>
	                          <span style="color: #CEE3F6; margin-left: -50px;">&nbsp&nbsp풍속 <i class="" id="todaysWind" style="color: #FFFFFF;"></i><p style="float:right; position:relative; left: -37px; padding-left:-50px; width: 20px; font-size: 17px; margin-top: 1px; color: #CEE3F6;">m/s</p></span>
	                      </div>
	                      </li>
	                      </ul>
	                </div>
...
<!--                     <a href="single-post.html" style="border-radius: 0; padding: 7px 25px 7px 25px;">Full Article</a> -->
					<a href=""
						style=" width:200px; color: #fff; background: #346D4E; border-radius: 20px; margin-left: -120px; margin-top: 10px; padding: 7px 25px 7px 25px;">보러가기</a>
				</div>
          </div>
<!-- 검역정보 -->		    
         <div class="wow fadeInUp col-md-6 col-sm-6" data-wow-delay="1s">
...
function loadJSON(callback) //url의 json 데이터 불러오는 함수
{
var apiURI = "http://api.openweathermap.org/data/2.5/weather?q=Daejeon&appid=230e19eb41d6add7a83c9372285ff2e5";
...
$.ajax({
 ...
 success: function(resp) {
     ...
     var temp_sub = String(temp).substring(0,2);
...
// 		console.log((resp.main.temp- 273.15));
		$("#todaysTemp").text( temp_sub );
		$("#todaysWind").text( resp.wind.speed );
...
$.ajax({
	...
	success : function(data) {
		...
		if(data.length == 0){
			...
			text += "<h2 id=\"gumyuk\" style=\"color=\">	<strong>" + data[0].areaNm + "</strong>,예측경보 발생 병해충</h2>";
