...
<div id="" style="position: relative; top: 70px;">
	<div class="">
		<ul >
			<li style="position: relative; left: 0px;  font-size: 15px; color: #0B3B24; float: left;">고객센터 ></li>
			<li style="position: relative; left: 10px; top: -5px; font-size: 20px; color: #0B3B24; float: left;"> 고객센터 수정</li>
		</ul>
                ...
                <div class="col-lg-8">
                    <div class="p-ins">
                        ...
                        <ul style="margin-top: 50px;">
	                                        	...
	                                        	<li style="float:left;  padding-left: 10px;"> <label class="" for="cstmr_cnter_sj" style="">제 목</label></li>
	                                       		<li style="float:left; padding-left: 18px;"> <input type="text" class="form-control" id="cstmr_cnter_sj" name="cstmr_cnter_sj"  placeholder="제목 입력" style="position: relative; top:-5px; width: 100px;  height: 30px;"></li>
	                                        </ul>
	                     </div>
                        <div class="row">
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                ...
                                <div style=" position: relative; top: 5px;">
                                    <textarea rows="15" cols="95" class="cstmr_cnter_cn"  id="cstmr_cnter_cn" name="cstmr_cnter_cn" placeholder="내용을 입력하세요" >${customerCenterInfo.cstmr_cnter_cn}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
	...
	<div style="float: left; position: absolute; left: 110px; top: 600px;">
...
var writr =  '${customerCenterInfo.mber_id}';
	CKEDITOR.replace('cstmr_cnter_cn'
	        , {height: 200                                                 
	         });
$(function(){
	$('#cstmr_cnter_sj').val('${customerCenterInfo.cstmr_cnter_sj}');
	$('#mber_ncnm').val('${customerCenterInfo.mber_ncnm}');
	
// 	DB에서 textarea를 <br> 없이 가져오는거 
	 var str = $('#cstmr_cnter_cn').val();
	 str = str.split('<br/>').join("\r\n");
	 $('#cstmr_cnter_cn').val(str);
	 ...
	 $('form[name=updateForm]').submit(function(){
	      ...
	      if($('#cstmr_cnter_sj').val() == null || $('#cstmr_cnter_sj').val().trim() == ""){
		         alert("제목을 입력하세요");
		         return false;
		      }
	      ...
	      $(this).attr('action', '${pageContext.request.contextPath}/user/customercenter/updateCustomerCenter.do'); 
...
// 	      DB로 엔터값 처리해서 저장
	      ...
	      var cstmr_cnter_sj = $('#ctvt_mth_sj').val();
	      var mber_ncnm = $('#mber_ncnm').val();
	      var cstmr_cnter_cn = $('#cstmr_cnter_cn').val(str);
	      $(this).append('<input type ="hidden" name="cstmr_cnter_no" value="${customerCenterInfo.cstmr_cnter_no}"></input>');
	      $(this).append('<input type="hidden" name="mber_id" value="${customerCenterInfo.mber_id}"></input>'); //<==form 셀렉팅하기
...
// 	      $(this).append('<textarea type="hidden" name="ctvt_mth_cn"></textarea>' );
	...
	$('#cancelbt').click(function() {
		var cstmr_cnter_no = '${cstmr_cnter_no}';
		var search_keyword = encodeURIComponent('${search_keyword}');
		var search_keycode = '${search_keycode}';
		var currentPage = '${currentPage}';
		$(location).attr('href', '/user/customercenter/customerCenterView.do?cstmr_cnter_no=' + cstmr_cnter_no + '&search_keyword=' + search_keyword + '&search_keycode=' +search_keycode+ '&currentPage=' + currentPage);
	});
	$('#listbt').click(function() {
		var search_keyword = encodeURIComponent('${search_keyword }');
	    var search_keycode = '${search_keycode }';
	    var currentPage = '${currentPage }';
		$(location).attr('href', '/user/customercenter/customerCenterList.do?search_keyword='+search_keyword+'&search_keycode='+search_keycode+'&currentPage='+currentPage);
	});
